...
//业务中post数据 , 若无则为空
$post_data = isset($req_post['post_data']) ? $req_post['post_data'] : array() ;

//默认使用异步请求
$req_type  = isset($req_post['type']) ? $req_post['type'] : 'async' ;
...
if($req_type == 'async') {
    ...
    $ssdb->hset($queue_name, $flag, base64_encode(json_encode($req_post)));
    ...
    //记录慢响应
    if($hook_conf['slow_log']){
        $GLOBALS['t_start'] = microtime_float();
    }

    //发送异步post请求
    $cli->post($req_path, $post_data, function ($cli) {
        $t_start = $GLOBALS['t_start'] ;
        $req_post = $GLOBALS['req_post'] ;
        /**
        ...
        if($hook_conf['slow_log']){
            ...
            if($t_diff > $hook_conf['slow_log_tm'] ) {
                if($hook_conf['slow_log_cron'] == 'm' ){
                    $log_set_name = 'slow_log_q_'.date('Y-m') ;
                }
                elseif($hook_conf['slow_log_cron'] == 'd' ){
                    $log_set_name = 'slow_log_q_'.date('Y-m-d') ;
                }

                //记录慢响应数据包
                $ssdb -> zset($log_set_name, base64_encode(json_encode($req_post)) , time() ) ;
            }
        }
...
//使用同步请求获取数据
    ...
    $rtn_data = curl_post($req_post['url'], $post_data, $serv_conf['version']) ;
